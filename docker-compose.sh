...
Commands:
  up              Start WAHA with internal Redis (recommended)
  up-host         Start WAHA with host network (for external Redis)
  up-redis        Start only Redis service
  down            Stop and remove all services
  clean           Clean up existing containers (run this if port conflicts occur)
  restart         Restart all services
  logs            Show logs from all services
  logs-waha       Show logs from WAHA service only
...
Examples:
  $0 up                    # Start services with internal Redis
  $0 up-host               # Start services with external Redis
